...
![Дашборд ChronoTrack — обзор дня](screenshots/demo-overview.png)
...
![Настройки ChronoTrack](screenshots/demo-settings.png)
...
Замер на обычном домашнем ПК (окно 60 секунд, uptime 7,5 часов):

![Нагрузка на систему](screenshots/system-load.jpg)

<sub>Сводка замера: метрики, сравнение CPU с фоновым табом Chrome и рост данных на диске.</sub>

| Метрика | Значение |
|---|---|
| CPU | **0,03%** системы (≈0,36% одного ядра) |
| RAM | **~90 МБ** (0,5% от 16 ГБ) |
| Диск (I/O) | **~10 КБ/с** суммарно (чтение + запись) |
| Один цикл опроса | **~18 мс** раз в 5 секунд |
| Потоки / хендлы | 11 / ~460 |
| Сеть в покое | ≈0 — порт 8777 просто слушает |

Для наглядности: один бездействующий фоновый таб Chrome потребляет
~1,45% CPU — примерно **в 18 раз больше**, чем ChronoTrack (~0,08%).

Рост данных на диске со временем:

| Период | База статистики | База + кэши (Steam, Dota) |
|---|---|---|
| Неделя | несколько МБ | несколько МБ |
| Месяц | ~10 МБ | ~15 МБ |
| Год | ~205 МБ | ~235 МБ |

За 7,5 часа непрерывной работы база выросла всего до 1,93 МБ
(WAL-файл — до ~4 МБ): записи крошечные и открываются только при смене
состояния окна, а не на каждом опросе.
