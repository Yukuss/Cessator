...
1. Скачайте `Cessator-Setup.exe` из [Releases](../../releases).
...
3. После установки в трее появится значок, а дашборд откроется на
   `http://cessator.local` (или `http://127.0.0.1:8777`).

Обновление - просто запустите новый `Setup.exe` поверх старого:
данные, ключи и настройки сохраняются. Установленные версии ChronoTrack
обновляются так же - статистика, ключи и автозапуск переезжают сами.
